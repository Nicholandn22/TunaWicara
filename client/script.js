...
async function mulaiRekam(kata) {
  ...
  mediaRecorder.onstop = () => {
    ...
    localStorage.setItem(`${currentUser}_${currentKata}`, "recorded");
...
async function kirimSemua() {
  ...
  for (const rekaman of rekamanList) {
    try {
      ...
      formData.append("jenis_sakit", jenis_sakit);
  ...
  await loadUnrecordedKata(); // Refresh tampilan berdasarkan DB
  alert("Proses upload selesai!");
}

// Inisialisasi user dan load data
document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    if (!res.ok) throw new Error("Gagal ambil userId");

    const data = await res.json();
    userId = data.userId;
    jenis_sakit = data.jenis_sakit;
    document.getElementById("userId").textContent = userId;

    await loadUnrecordedKata(); // ⬅️ Ambil kata dari database
  } catch (err) {
...
let unrecordedKataList = []; // ← Digunakan sebagai pengganti filter manual dari localStorage

async function loadUnrecordedKata() {
  if (!userId) return;
  try {
    const response = await fetch(`/unrecorded-kata?user_id=${userId}`);
    if (!response.ok) throw new Error("Gagal mengambil data rekaman");

    const data = await response.json();
    const recordedKataSet = new Set(data.recordedKata);

    // Filter hanya kata yang belum direkam oleh user
    unrecordedKataList = kataList.filter((kata) => !recordedKataSet.has(kata));

    renderKataList();
  } catch (err) {
    console.error("Gagal memuat kata dari DB:", err);
  }
}

function renderKataList() {
  console.log("Rendering kata list..."); // Tambahkan ini
  console.log("Unrecorded kata:", unrecordedKataList); // Ini juga
  container.innerHTML = "";
  const start = (currentPage - 1) * itemsPerPage;
  const end = start + itemsPerPage;
  const currentItems = unrecordedKataList.slice(start, end);

  currentItems.forEach((kata) => {
    const div = document.createElement("div");
    const sudahTerekamDariDB = !unrecordedKataList.includes(kata);
    const sudahDirekamSementara = rekamanList.some((r) => r.kata === kata);
    const isRecorded = sudahTerekamDariDB || sudahDirekamSementara;
...
function renderPaginationButtons() {
  ...
  const totalPages = Math.ceil(unrecordedKataList.length / itemsPerPage);
